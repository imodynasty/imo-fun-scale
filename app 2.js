...
function renderSummary(){
  ...
  const leaderAvatar=$("leaderAvatar");if(leaderAvatar){leaderAvatar.innerHTML=lead?.avatar?`<img src="${esc(lead.avatar)}" alt="" loading="lazy">`:`<span>${esc(lead?.initials||"—")}</span>`}
  $("leaderTrades").textContent=lead?.count||0;$("leaderWindow").textContent=activeWindowLabel(state.activeWindow);
  $("latestTradeShort").innerHTML=latest?tradeAssets(latest)&&Object.values(tradeAssets(latest)).flat().filter(a=>a.type==='player').slice(0,3).map(a=>playerLink(a.id,a.name,'summary-player-link')).join('<span class="summary-player-separator"> / </span>')||esc(tradeSummary(latest)):"—";$("latestTradeDate").textContent=latest?`${mids(latest).map(id=>managerName(id,latest)).join(" ↔ ")} · ${fmt(latest.created)}`:"No trade"
}
...
const ARCHETYPE_GUIDE=[
  {icon:'🏗️',name:'The Rebuilder',copy:'Operating with a long horizon, this front office prioritizes young talent, developmental upside, and future roster flexibility over short-term wins.'},
  {icon:'🤝',name:'The Wheel & Dealer',copy:'One of the most active front offices in the league. Always testing the market, floating offers, and continually reshaping the roster through trades.'},
  {icon:'🦖',name:'The Veteran Chaser',copy:'Built around proven, immediate production. Prefers established veterans with proven track records who can contribute win-now points right away.'},
  {icon:'🏆',name:'The Contender',copy:'Assembled explicitly to win a championship right now. Boasts high-end production across the roster with a clear focus on maximizing their title window.'},
  {icon:'🎯',name:'The Talent Collector',copy:'Concentrates top-tier, elite talent at the very top of the roster. Built around superstar players capable of deciding weekly matchups single-handedly.'},
  {icon:'💎',name:'Draft Capital King',copy:'A premier asset accumulator who holds a stockpile of future draft picks, maintaining ultimate leverage and long-term optionality.'},
  {icon:'🛡️',name:'Diamond Hands',copy:'Runs a patient, steady front office. Values roster continuity, chemistry, and calculated long-term moves over constant turnover and roster churn.'},
  {icon:'🌱',name:'Prospect Hunter',copy:'Committed to a youth-led build. Consistently targets young players and emerging prospects whose best fantasy years are still ahead of them.'},
  {icon:'🎲',name:'The Gambler',copy:'Embraces volatility and variance. Comfortable taking on high-risk, high-reward moves and blockbuster trades in pursuit of a massive payoff.'}
];
function ensureArchetypeGuideStyles(){
  if(document.getElementById('archetypeGuideStyles'))return;
  const style=document.createElement('style');
  style.id='archetypeGuideStyles';
  style.textContent=`
    .profile-hero-actions{margin-top:10px}
    .archetype-guide-btn{appearance:none;border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.08);color:inherit;border-radius:999px;padding:7px 12px;font:inherit;font-size:12px;font-weight:800;letter-spacing:.02em;cursor:pointer;transition:.18s ease}
    .archetype-guide-btn:hover,.archetype-guide-btn:focus-visible{background:rgba(255,255,255,.15);transform:translateY(-1px);outline:none}
    .archetype-guide-modal{position:fixed;inset:0;z-index:10050;display:none;align-items:center;justify-content:center;padding:20px}
    .archetype-guide-modal.open{display:flex}
    .archetype-guide-backdrop{position:absolute;inset:0;border:0;background:rgba(5,10,20,.74);backdrop-filter:blur(8px);cursor:pointer}
    .archetype-guide-card{position:relative;width:min(760px,100%);max-height:min(84vh,900px);overflow:auto;border-radius:22px;background:var(--card-bg,#111827);color:var(--text,#fff);box-shadow:0 28px 90px rgba(0,0,0,.5);padding:22px}
    .archetype-guide-head{display:flex;align-items:flex-start;justify-content:space-between;gap:18px;margin-bottom:16px}
    .archetype-guide-head h3{margin:3px 0 0;font-size:24px}
    .archetype-guide-close{border:0;background:rgba(255,255,255,.1);color:inherit;width:38px;height:38px;border-radius:50%;font-size:24px;cursor:pointer}
    .archetype-guide-list{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}
    .archetype-guide-item{display:grid;grid-template-columns:42px 1fr;gap:12px;align-items:flex-start;padding:15px;border:1px solid rgba(255,255,255,.1);background:rgba(255,255,255,.045);border-radius:16px}
    .archetype-guide-icon{display:grid;place-items:center;width:42px;height:42px;border-radius:13px;background:rgba(255,255,255,.08);font-size:23px}
    .archetype-guide-item strong{display:block;font-size:15px;margin-bottom:5px}
    .archetype-guide-item p{margin:0;opacity:.76;line-height:1.5;font-size:13px}
    body.archetype-guide-open{overflow:hidden}
    @media(max-width:620px){
      .archetype-guide-modal{align-items:flex-end;padding:0}
      .archetype-guide-card{width:100%;max-height:88vh;border-radius:22px 22px 0 0;padding:18px 16px 24px}
      .archetype-guide-list{grid-template-columns:1fr}
      .archetype-guide-head h3{font-size:21px}
      .profile-hero-actions{margin-top:8px}
      .archetype-guide-btn{padding:7px 11px;font-size:11px}
    }
  `;
  document.head.appendChild(style);
}
function openArchetypeGuide(){
  ensureArchetypeGuideStyles();
  let modal=document.getElementById('archetypeGuideModal');
  if(!modal){
    modal=document.createElement('div');
    modal.id='archetypeGuideModal';
    modal.className='archetype-guide-modal';
    modal.setAttribute('aria-hidden','true');
    modal.innerHTML=`<button type="button" class="archetype-guide-backdrop" data-close-archetype-guide aria-label="Close Archetype Guide"></button><section class="archetype-guide-card" role="dialog" aria-modal="true" aria-labelledby="archetypeGuideTitle"><div class="archetype-guide-head"><div><span class="eyebrow">GM PROFILE GUIDE</span><h3 id="archetypeGuideTitle">Archetype Guide</h3></div><button type="button" class="archetype-guide-close" data-close-archetype-guide aria-label="Close">×</button></div><div class="archetype-guide-list">${ARCHETYPE_GUIDE.map(x=>`<article class="archetype-guide-item"><span class="archetype-guide-icon">${x.icon}</span><div><strong>${esc(x.name)}</strong><p>${esc(x.copy)}</p></div></article>`).join('')}</div></section>`;
    document.body.appendChild(modal);
  }
  modal.classList.add('open');
  modal.setAttribute('aria-hidden','false');
  document.body.classList.add('archetype-guide-open');
  requestAnimationFrame(()=>modal.querySelector('.archetype-guide-close')?.focus());
}
function closeArchetypeGuide(){
  const modal=document.getElementById('archetypeGuideModal');
  if(modal){modal.classList.remove('open');modal.setAttribute('aria-hidden','true')}
  document.body.classList.remove('archetype-guide-open');
}
...
document.addEventListener("click",e=>{
  if(e.target.closest('[data-open-archetype-guide]')){e.preventDefault();e.stopPropagation();openArchetypeGuide();return}
  if(e.target.closest('[data-close-archetype-guide]')){e.preventDefault();closeArchetypeGuide();return}
