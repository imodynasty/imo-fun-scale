/* IMO DYNASTY V2.6.2 — homepage simplification */
...
function renderPower(){const weeks=meaningfulWeeks(state.modelBundle),through=weeks.at(-1)||Infinity,prior=weeks.length>1?weeks.at(-2):through,p=modelRows(state.modelBundle,through,"power"),oddsById=Object.fromEntries(priceRows(through).map(x=>[String(x.id),x])),oldOdds=Object.fromEntries(priceRows(prior).map(x=>[String(x.id),x])),n=Math.max(p.length-1,1);$("powerRankings").classList.remove("loading");$("powerRankings").innerHTML=p.map((x,i)=>{const oldRank=state.previousPowerRanks[x.id]??x.rank,diff=oldRank-x.rank,arrow=diff>0?`<span class="movement up">▲ ${diff}</span>`:diff<0?`<span class="movement down">▼ ${Math.abs(diff)}</span>`:`<span class="movement flat">—</span>`,hue=140-(140*i/n),m=state.managers.get(String(x.id)),avatar=m?.avatar?`<img src="${esc(m.avatar)}" alt="" loading="lazy">`:`<span>${esc(m?.initials||x.name.slice(0,2).toUpperCase())}</span>`,o=oddsById[String(x.id)]||x,prev=oldOdds[String(x.id)]?.odds??o.odds,delta=o.odds-prev,oddsClass=delta<0?'up':delta>0?'down':'flat',moveText=prev===o.odds?'No price movement':`$${prev.toFixed(2)} → $${o.odds.toFixed(2)}`,ladderMove=(oldOdds[String(x.id)]?.standingRank||o.standingRank)-o.standingRank,ladderText=ladderMove>0?`Moved up to ${ordinal(o.standingRank)} on ladder`:ladderMove<0?`Dropped to ${ordinal(o.standingRank)} on ladder`:`Currently ${ordinal(o.standingRank)} on ladder`,streak=o.streak>=1?`Won last ${o.streak}`:`Form: ${Math.round(o.form*5)} wins from last 5`;return `<details class="power-odds-row" style="--rank-colour:hsl(${hue} 85% 52%)"><summary><b class="power-rank-number">${x.rank}</b><span class="power-avatar">${avatar}</span><span class="power-copy"><button class="power-name manager-profile-link" type="button" data-manager-id="${esc(x.id)}">${esc(x.name)}</button><small>View manager profile</small></span><span class="power-odds-price">$${Number(o.odds||0).toFixed(2)}</span>${arrow}<span class="power-chevron">⌄</span></summary><div class="power-detail"><div>${o.streak>=3?'🔥':o.streak?'✅':'⚪'} ${streak}</div><div>🔥 Averaging ${Number(o.avg5||0).toFixed(1)} over last five weeks</div><div>${ladderMove<0?'⬇':'⬆'} ${ladderText}</div><div class="odds-move ${oddsClass}">${moveText}</div></div></details>`}).join("")}
...
function fallbackSeasonFormRows(){
  ...
  if(!bundle)return [];
  const weeks=meaningfulWeeks(bundle);if(weeks.length<3)return [];
  const recentWeeks=new Set(weeks.slice(-2)),priorWeeks=new Set(weeks.slice(0,-2)),recentSums={},recentGames={},priorSums={},priorGames={};
  matchupRows(bundle).forEach(row=>Object.entries(row.players_points||{}).forEach(([id,value])=>{const pts=Number(value);if(!Number.isFinite(pts)||pts<=0)return;if(recentWeeks.has(row.week)){recentSums[id]=(recentSums[id]||0)+pts;recentGames[id]=(recentGames[id]||0)+1}else if(priorWeeks.has(row.week)){priorSums[id]=(priorSums[id]||0)+pts;priorGames[id]=(priorGames[id]||0)+1}}));
  return [...new Set([...Object.keys(priorSums),...Object.keys(recentSums)])].filter(id=>priorGames[id]&&recentGames[id]).map(id=>{const priorAvg=priorSums[id]/priorGames[id],recentAvg=recentSums[id]/recentGames[id];return{id,name:playerName(id),priorAvg,recentAvg,change:recentAvg-priorAvg,sourceSeason:"2025"}})
}
function allGameLogFormRows(){
  ...
  for(const season of preferred){
    const logs=state.gameLogs?.[season]||{},dates=Object.values(logs).flat().map(gameDateValue).filter(Boolean);if(!dates.length)continue;
    const latest=new Date(Math.max(...dates.map(d=>d.getTime()))),recentStart=new Date(latest.getTime()-14*864e5),rows=[];
    Object.entries(logs).forEach(([id,games])=>{let priorPts=0,priorGames=0,recentPts=0,recentGames=0;(games||[]).forEach(row=>{const date=gameDateValue(row),fpts=rawFantasyPoints(row,state.modelBundle?.league?.scoring_settings||{});if(!date||!gameWasPlayed(row)||!Number.isFinite(fpts))return;if(date>=recentStart){recentPts+=fpts;recentGames++}else{priorPts+=fpts;priorGames++}});if(priorGames&&recentGames){const priorAvg=priorPts/priorGames,recentAvg=recentPts/recentGames;rows.push({id,name:playerName(id),priorAvg,recentAvg,change:recentAvg-priorAvg,sourceSeason:season})}});
...
function favouriteTradePartners(id){const counts={};managerTrades(id).forEach(t=>mids(t).filter(x=>x!==String(id)).forEach(x=>counts[x]=(counts[x]||0)+1));return Object.entries(counts).map(([partner,count])=>({partner,count})).sort((a,b)=>b.count-a.count).slice(0,7)}
...
function managerProfileHTML(managerId){
  ...
  const partnerRows=partners.map((p,i)=>`<div class="profile-partner-row"><b>${i+1}</b><button class="manager-profile-link" type="button" data-manager-id="${esc(p.partner)}">${esc(managerName(p.partner))}</button><span>${p.count} trades</span></div>`).join("")||'<div class="profile-empty">No trade partners yet.</div>';
  const h2hRows=headToHead.map(r=>`<div class="profile-h2h-row"><button class="manager-profile-link" type="button" data-manager-id="${esc(r.oppId)}">vs ${esc(managerName(r.oppId))}</button><strong>${r.wins}-${r.losses}${r.draws?`-${r.draws}`:""}</strong><small>${r.games} games</small></div>`).join("")||'<div class="profile-empty">No completed head-to-head matchups.</div>';
