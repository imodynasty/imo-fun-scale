/* IMO DYNASTY V2.7.2 — weekly GM archetypes, 2K-style tendencies and scouting reports */
...
function priceRowsForBundle(bundle,through){const noCurrentGames=String(bundle.league?.league_id)===CONFIG.currentLeagueId&&meaningfulWeeks(bundle).length===0,rows=noCurrentGames?preseasonProjectionRows(bundle):modelRows(bundle,through,"odds"),strengths=rows.map(x=>Math.pow(CONFIG.oddsBaseline+(1-CONFIG.oddsBaseline)*Math.max(0,Math.min(1,x.score)),CONFIG.oddsExponent)),sum=strengths.reduce((a,b)=>a+b,0)||1;return rows.map((x,i)=>{const fair=strengths[i]/sum,market=Math.min(.99,fair*CONFIG.bookmakerMargin),odds=roundFive(Math.min(CONFIG.maxDisplayedOdds,Math.max(1.01,1/market)));return {...x,odds,probability:fair,eliminated:false}})}
...
function managerTendencyLeague(){
  ...
  return{ratings,raw,week:weeks.at(-1)||0}
}
...
function managerScoutReport(name,r,primary,secondary){
  const high=Object.entries(r).sort((a,b)=>b[1]-a[1]),low=[...high].reverse(),labels={trade:'trade market',youth:'young assets',winNow:'win-now production',draft:'draft capital',waiver:'waiver wire',risk:'high-variance moves',asset:'asset management',star:'star power'};
  const opening=`${name} profiles as ${primary.name.toLowerCase()}, with a secondary lean toward ${secondary.name.toLowerCase()}.`;
  let middle=`The front office's calling cards are ${labels[high[0][0]]} and ${labels[high[1][0]]}.`;
  let close=low[0][1]<40?`The clearest opportunity is to improve its approach to ${labels[low[0][0]]}.`:`There are no major inactive areas, making this one of the league's more balanced management profiles.`;
  if(r.trade>=85)middle=`Few managers reshape a roster more aggressively; activity in the ${labels.trade} is central to every plan.`;
  if(r.draft>=85&&r.youth>=70)middle=`This is a future-facing operation built around ${labels.draft} and ${labels.youth}.`;
  if(r.winNow>=85&&r.star>=70)middle=`This roster is constructed to win now, leaning heavily on elite production and ${labels.star}.`;
