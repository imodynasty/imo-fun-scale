...
function playerFptsPer36(playerId,season=null,averageOverride=null){
  ...
  return{season:resolvedSeason,average,mpg,totalMinutes,value:Number.isFinite(value)?value:0,eligible,monster:eligible&&value-average>=5};
}
...
function buildGlobalSearchIndex(){
  const items=[];
  Object.keys(state.players||{}).forEach(id=>{const name=playerName(id);if(!name||/^Player \d+$/.test(name))return;const p=state.players[id]||{},current=playerCurrentAverage(id),efficiency=playerFptsPer36(id,current.season,current.avg),identity=[p.team,p.position].filter(Boolean).join(' · ')||'Player profile',stats=current.avg>0?` · ${current.avg.toFixed(2)} FPTS/G · ${efficiency.mpg>0?efficiency.mpg.toFixed(1):'—'} MPG · ${fpts36Display(efficiency)} FPTS/36`:'';items.push({type:'Player',title:name,subtitle:`${identity}${stats}`,search:`${name} ${p.team||''} ${p.position||''} ${current.avg.toFixed(2)} ${fpts36Display(efficiency)} FPTS per 36`,action:'player',id:String(id)})});
