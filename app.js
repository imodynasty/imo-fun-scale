/* IMO DYNASTY V2.6.4 — form, head-to-head and badge update */
...
function managerLuckRating(managerId,bundle=state.modelBundle,throughWeek=Infinity){
  if(!bundle)return 0;
  const rows=matchupRows(bundle,throughWeek),byWeek={};rows.forEach(x=>(byWeek[x.week]??=[]).push(x));let actual=0,expected=0,games=0;
  Object.values(byWeek).forEach(weekRows=>{const valid=weekRows.filter(x=>Number.isFinite(Number(x.points))&&Number(x.points)>0),mine=valid.find(x=>String(bundle.ownerByRoster?.[String(x.roster_id)])===String(managerId));if(!mine||valid.length<2)return;const minePts=Number(mine.points),opponents=valid.filter(x=>x!==mine),lower=opponents.filter(x=>minePts>Number(x.points)).length,equal=opponents.filter(x=>minePts===Number(x.points)).length;expected+=(lower+equal*.5)/opponents.length;games++});
  const outcomes=outcomesForBundle(bundle,throughWeek)[String(managerId)]||[];actual=outcomes.reduce((sum,g)=>sum+g.result,0);return games?actual-expected:0
}
