...
function renderPower(){
  ...
  const powerRows=modelRows(state.modelBundle,through,"power"),oddsById=Object.fromEntries(priceRows(through).map(x=>[String(x.id),x])),oldOdds=Object.fromEntries(priceRows(prior).map(x=>[String(x.id),x]));
  const n=Math.max(powerRows.length-1,1),target=$("powerRankings");
  target.classList.remove("loading");
  target.innerHTML=powerRows.map((x,i)=>{
    const id=String(x.id),o=oddsById[id],prev=oldOdds[id]?.odds??o?.odds,delta=o&&prev!=null?o.odds-prev:0,cls=delta<0?'up':delta>0?'down':'flat';
    const oldRank=state.previousPowerRanks[id]??x.rank,diff=oldRank-x.rank,rankMove=diff>0?`▲ ${diff}`:diff<0?`▼ ${Math.abs(diff)}`:'—';
    const m=state.managers.get(id),avatar=m?.avatar?`<img src="${esc(m.avatar)}" alt="" loading="lazy">`:`<span>${esc(m?.initials||x.name.slice(0,2).toUpperCase())}</span>`;
    const streak=o?.streak>=1?`Won last ${o.streak}`:`${Math.round((o?.form||0)*5)} wins from last 5`;
    const ladderMove=(oldOdds[id]?.standingRank||o?.standingRank)-(o?.standingRank||0),ladderText=ladderMove>0?`Moved up to ${ordinal(o.standingRank)} on the ladder`:ladderMove<0?`Dropped to ${ordinal(o.standingRank)} on the ladder`:`Currently ${ordinal(o?.standingRank||x.standingRank)} on the ladder`;
    const moveText=!o||prev===o.odds?'No odds movement':`$${prev.toFixed(2)} → $${o.odds.toFixed(2)}`;
    const hue=140-(140*i/n);
    return `<article class="mega-power-row" style="--rank-colour:hsl(${hue} 85% 52%)">
      <button class="mega-power-main manager-profile-link" type="button" data-manager-id="${esc(id)}" aria-label="Open ${esc(x.name)} manager profile">
        <b class="mega-power-rank">${x.rank}</b><span class="power-avatar">${avatar}</span><span class="mega-power-team"><strong>${esc(x.name)}</strong><small>Power rank ${rankMove}</small></span><span class="mega-power-odds"><small>Championship odds</small><strong>${o?`$${o.odds.toFixed(2)}`:'—'}</strong></span>
      </button>
      <details class="mega-power-notes"><summary aria-label="Show championship odds notes"><span>⌄</span></summary><div class="mega-power-detail"><div>${streak}</div><div>Averaging ${o?.avg5?.toFixed?.(1)??'—'} over the last five weeks</div><div>${ladderText}</div><div class="${cls}">${moveText}</div></div></details>
    </article>`
  }).join("")
}
function renderOdds(){if(!$("championshipOdds"))return;const weeks=meaningfulWeeks(state.modelBundle),through=weeks.at(-1)||Infinity,prior=weeks.length>1?weeks.at(-2):through,rows=priceRows(through),old=Object.fromEntries(priceRows(prior).map(x=>[x.id,x]));const nonFav=rows.filter((_,i)=>i!==0).map(x=>({id:x.id,improvement:(old[x.id]?.odds??x.odds)-x.odds})).sort((a,b)=>b.improvement-a.improvement),smokeyId=nonFav[0]?.improvement>0?nonFav[0].id:null;$("championshipOdds").classList.remove("loading");$("championshipOdds").innerHTML=rows.map((x,i)=>{const prev=old[x.id]?.odds??x.odds,delta=x.odds-prev,cls=delta<0?'up':delta>0?'down':'flat',icon=delta<0?'📈':delta>0?'📉':'➖',streak=x.streak>=1?`Won last ${x.streak}`:`Form: ${Math.round(x.form*5)} wins from last 5`,ladderMove=(old[x.id]?.standingRank||x.standingRank)-x.standingRank,ladderText=ladderMove>0?`Moved up to ${ordinal(x.standingRank)} on ladder`:ladderMove<0?`Dropped to ${ordinal(x.standingRank)} on ladder`:`Currently ${ordinal(x.standingRank)} on ladder`,moveText=prev===x.odds?'No price movement':`$${prev.toFixed(2)} → $${x.odds.toFixed(2)}`,tag=i===0?'<span class="odds-tag favourite-tag"><span>★</span> Favourite</span>':x.id===smokeyId?'<span class="odds-tag smokey-tag"><span>🔥</span> Smokey</span>':'';return `<details class="odds-row-wrap"><summary class="odds-summary"><b>${i+1}</b><div class="odds-summary-main"><strong>${esc(x.name)}</strong>${tag}</div><div class="odds-price-stack"><span class="odds">$${x.odds.toFixed(2)}</span><div class="odds-move ${cls}">${icon} ${moveText}<span class="odds-chevron">▼</span></div></div></summary><div class="odds-detail"><div>${x.streak?`${x.streak>=3?'🔥':'✅'} ${streak}`:`⚪ ${streak}`}</div><div>🔥 Averaging ${x.avg5.toFixed(1)} over last five weeks</div><div>${ladderMove<0?'⬇':'⬆'} ${ladderText}</div></div></details>`}).join("")}
...
function recentPlayerForm(){let rows=allGameLogFormRows();if(!rows.length){const prior=seasonAverageMap("2025"),recent=seasonAverageMap("2026");rows=Object.keys(recent).filter(id=>Number(recent[id])>0&&Number(prior[id])>0).map(id=>({id,name:playerName(id),priorAvg:Number(prior[id]),recentAvg:Number(recent[id]),change:Number(recent[id])-Number(prior[id])}))}return{poor:rows.filter(x=>x.priorAvg>=20&&x.recentAvg>0&&x.change<0).sort((a,b)=>a.change-b.change).slice(0,5),good:rows.filter(x=>x.priorAvg>=15&&x.recentAvg>0&&x.change>0).sort((a,b)=>b.change-a.change).slice(0,5)}}
function renderFormList(targetId,rows,positive){const target=$(targetId);target.classList.remove("loading");target.innerHTML=rows.length?rows.map((x,i)=>`<div class="rank-item form-player-row ${positive?'good-form-row':'poor-form-row'}"><b>${i+1}</b><div>${playerLink(x.id,x.name,"form-player-link")}<div class="meta">${x.priorAvg.toFixed(1)} previously → ${x.recentAvg.toFixed(1)} last 2 weeks</div></div><span class="form-change ${positive?'positive':'negative'}">${x.change>0?'+':''}${x.change.toFixed(1)}</span></div>`).join(""):`<div class="block-empty">No qualifying form changes are available yet.</div>`}
...
function favouriteTradePartners(id){const counts={};managerTrades(id).forEach(t=>mids(t).filter(x=>x!==String(id)).forEach(x=>counts[x]=(counts[x]||0)+1));return [...state.managers.keys()].filter(x=>String(x)!==String(id)).map(partner=>({partner:String(partner),count:counts[String(partner)]||0})).sort((a,b)=>b.count-a.count||managerName(a.partner).localeCompare(managerName(b.partner))).slice(0,8)}
...
function managerProfileHTML(managerId){
  ...
  const rosterRows=roster.map((p,i)=>`<div class="profile-roster-row"><span class="profile-roster-rank">${i+1}</span><span class="player-avatar-wrap"><img src="${esc(p.avatar)}" alt="" loading="lazy" onerror="this.style.display='none';this.nextElementSibling.style.display='grid'"><span class="player-avatar-fallback">${esc(p.name.split(/\s+/).map(x=>x[0]).slice(0,2).join(""))}</span></span><div>${playerLink(p.id,p.name,"profile-player-name")}<small>${esc(p.position)}${p.age?` · Age ${p.age}`:""}</small></div><b>${p.avg.toFixed(2)}</b></div>`).join("")||'<div class="profile-empty">No current roster data.</div>';
  ...
  <div class="manager-profile-grid">
    ...
    <section class="manager-profile-card"><div class="manager-profile-card-heading"><div><span class="eyebrow">TRADE DNA</span><h3>League Trade Partners</h3></div></div><div class="profile-partner-list">${partnerRows}</div></section>
