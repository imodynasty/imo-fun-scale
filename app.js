/* IMO DYNASTY V3.3.27 — Rookie Draft Targets & Target Badge Update */
...
function rookieDraftTargetCardHTML(row){
  const prospect=row?.prospect||{},name=String(prospect.name||'TBD'),initials=name.split(/\s+/).filter(Boolean).slice(0,2).map(part=>part[0]).join('').toUpperCase();
  return `<article class="rookie-target-card"><div class="rookie-target-headshot" aria-hidden="true"><span>${esc(initials||'—')}</span></div><div class="rookie-target-copy"><span class="rookie-target-slot">Projected #${Number(row?.overall)||'—'} overall</span><h4>${esc(name)}</h4><p>${esc(prospect.position||'—')} · ${esc(prospect.team||'—')}</p><span class="rookie-target-scout">${esc(rookieScoutTag(prospect))}</span></div></article>`;
}
...
function managerProfileHTML(managerId){
  ...
  <div class="manager-profile-tab-panels">
    ...
    <section class="manager-profile-tab-panel" data-manager-tab-panel="front-office"><div class="manager-profile-grid">${managerGrades}${rookieDraftTargets}<section class="manager-profile-card"><div class="manager-profile-card-heading"><div><span class="eyebrow">TRADE DNA</span><h3>Favourite Trade Partners</h3></div><span class="period-pill">Top 7</span></div><div class="profile-partner-list">${partnerRows}</div></section><section class="manager-profile-card"><div class="manager-profile-card-heading"><div><span class="eyebrow">TRANSACTIONS LOG</span><h3>Recent Trades</h3></div></div><div class="profile-trades-list">${recent.map(t=>managerTradeSummaryHTML(t,id)).join("")||'<div class="profile-empty">No trades found.</div>'}</div></section>${potentialTradeTargets}</div></section>
...
function managerProfileSessionKey(key){return `imo-profile-v3323-recovered-manager-profile|${managerProfileDataFingerprint()}|${key}`}
