...
function favouriteTradePartners(id){const counts={};managerTrades(id).forEach(t=>mids(t).filter(x=>x!==String(id)).forEach(x=>counts[x]=(counts[x]||0)+1));return [...state.managers.values()].filter(m=>m.id!==String(id)).map(m=>({partner:m.id,count:counts[m.id]||0})).sort((a,b)=>b.count-a.count||managerName(a.partner).localeCompare(managerName(b.partner))).slice(0,8)}
...
document.addEventListener("click",e=>{const oddsToggle=e.target.closest(".merged-odds-toggle");if(oddsToggle){e.preventDefault();e.stopPropagation();const detail=oddsToggle.closest(".merged-power-row");if(detail)detail.open=!detail.open;return}const mergedHit=e.target.closest(".merged-manager-hitarea");if(mergedHit){e.preventDefault();e.stopPropagation();openManagerProfile(mergedHit.dataset.managerId);return}
