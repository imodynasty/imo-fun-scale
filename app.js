...
async function downloadManagerShareCard(managerId,button){
  ...
  buttons.forEach(btn=>{btn.disabled=true;btn.classList.remove('is-success','is-error');btn.classList.add('is-loading');btn.textContent='Preparing…'});
  try{const canvas=await buildManagerShareCardCanvas(data);const blob=await new Promise((resolve,reject)=>canvas.toBlob(b=>b?resolve(b):reject(new Error('PNG export failed')),'image/png'));const url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`imo-dynasty-${fileSafeName(data.name)}-share-card.png`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1500);buttons.forEach(btn=>{btn.disabled=false;btn.classList.remove('is-loading','is-error');btn.classList.add('is-success');btn.textContent='Downloaded'});setTimeout(()=>reset('Download PNG'),1600)}catch(error){console.error('Failed to download manager share card',error);buttons.forEach(btn=>{btn.disabled=false;btn.classList.remove('is-loading','is-success');btn.classList.add('is-error');btn.textContent='Try again'});setTimeout(()=>reset('Download PNG'),1800)}}
...
function ensureManagerGradeStyles(){
  ...
  .manager-profile-hero-actions,.manager-profile-mobile-actions{display:flex;flex-wrap:wrap;gap:8px;align-items:center}.manager-profile-mobile-actions{display:none}.manager-share-card-btn{appearance:none;border:1px solid rgba(52,211,153,.42);background:rgba(16,185,129,.10);color:#d1fae5;border-radius:999px;padding:7px 12px;font:800 9px/1 inherit;letter-spacing:.06em;text-transform:uppercase;cursor:pointer;white-space:nowrap;display:inline-flex;align-items:center;justify-content:center;gap:6px;min-height:32px}.manager-share-card-btn:hover,.manager-share-card-btn:focus-visible{background:rgba(16,185,129,.18);border-color:rgba(52,211,153,.72);outline:none}.manager-share-card-btn.is-loading{opacity:.72;pointer-events:none}.manager-share-card-btn.is-success{background:rgba(34,197,94,.18);border-color:rgba(74,222,128,.72);color:#dcfce7}.manager-share-card-btn.is-error{background:rgba(239,68,68,.16);border-color:rgba(248,113,113,.72);color:#fee2e2}
  @media(max-width:680px){.manager-profile-hero-actions{display:none}.manager-profile-mobile-actions{display:flex;margin-top:14px}.manager-share-card-btn{min-height:44px;padding:0 15px;font-size:9px}.manager-share-card-btn-mobile{display:inline-flex!important}.manager-share-card-btn-desktop{display:none!important}}
...
function managerProfileSessionKey(key){return `imo-profile-v3213-four-draft-cards|${managerProfileDataFingerprint()}|${key}`}
