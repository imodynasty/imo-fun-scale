/* IMO DYNASTY V2.7.0 — protected superstar grades, awards dates and playoff-weighted odds */
...
function modelRows(bundle,throughWeek=Infinity,mode="power"){const standings=standingsTable(bundle,throughWeek),outcomes=outcomesForBundle(bundle,throughWeek),teamCount=Math.max(standings.length,1),completedWeeks=Number.isFinite(throughWeek)?throughWeek:(meaningfulWeeks(bundle).at(-1)||0),seasonProgress=Math.max(0,Math.min(1,completedWeeks/19));const raw=standings.map(team=>{const games=outcomes[team.id]||[],formN=mode==="odds"?5:3,recent=games.slice(-formN),last5=games.slice(-5),form=recent.length?recent.reduce((s,g)=>s+g.result,0)/recent.length:0,avg5=last5.length?last5.reduce((s,g)=>s+g.points,0)/last5.length:0,ladder=teamCount===1?1:1-(team.standingRank-1)/(teamCount-1),streak=winningStreak(games);return {...team,form,avg5,ladder,streak}});const avgs=raw.map(x=>x.avg5);return raw.map(x=>{const avgNorm=minMax(x.avg5,avgs,true);let score;if(mode==="odds"){const ladderWeight=.25+.50*seasonProgress,remaining=1-ladderWeight,formWeight=remaining*2/3,avgWeight=remaining/3;score=x.ladder*ladderWeight+x.form*formWeight+avgNorm*avgWeight}else score=x.ladder*.35+x.form*.35+avgNorm*.30;return {...x,score,seasonProgress}}).sort((a,b)=>b.score-a.score||b.avg5-a.avg5||a.name.localeCompare(b.name)).map((x,i)=>({...x,rank:i+1}))}
...
function priceRowsForBundle(bundle,through){const rows=modelRows(bundle,through,"odds"),week=Number.isFinite(through)?through:(meaningfulWeeks(bundle).at(-1)||0),strengths=rows.map(x=>week>=19&&x.standingRank>6?0:Math.pow(CONFIG.oddsBaseline+(1-CONFIG.oddsBaseline)*Math.max(0,Math.min(1,x.score)),CONFIG.oddsExponent)),sum=strengths.reduce((a,b)=>a+b,0)||1;return rows.map((x,i)=>{const eliminated=week>=19&&x.standingRank>6,fair=eliminated?0:strengths[i]/sum,market=eliminated?0:Math.min(.99,fair*CONFIG.bookmakerMargin),odds=eliminated?CONFIG.maxDisplayedOdds:roundFive(Math.min(CONFIG.maxDisplayedOdds,Math.max(1.01,1/market)));return {...x,odds,probability:fair,eliminated}})}
