...
function buildInsiderEdition(){
  const eventKey=insiderEventKey(),storageKey='imoInsiderEditionV3';
  try{const cached=JSON.parse(localStorage.getItem(storageKey)||'null');if(cached?.eventKey===eventKey)return cached}catch{}
  const current=state.bundles.find(b=>String(b.league?.league_id)===CONFIG.currentLeagueId)||state.modelBundle;
  const weeks=meaningfulWeeks(current),lastWeek=weeks.at(-1)||0,power=modelRows(state.modelBundle,weeks.at(-1)||Infinity,'power'),leader=power[0],latest=state.trades[0],topPlayer=currentTopPlayer(),streakText=tickerStreak(),rankingText=tickerRankingOrRecord();
  const streakRows=[];
  ...
  const stories=[];
  if(latest){const playerAssets=Object.values(tradeAssets(latest)).flat().filter(a=>a.type==='player');const hero=playerAssets.sort((a,b)=>Number(b.value||0)-Number(a.value||0))[0];const teams=mids(latest).map(id=>managerName(id,latest));stories.push({kicker:'BLOCKBUSTER',headline:`${hero?.name||'Latest deal'} sends a jolt through IMO Dynasty`,image:hero?insiderPlayerImage(hero.id):state.managers.get(mids(latest)[0])?.avatar||null,body:`${teams.join(' and ')} have reset the conversation around the league after completing the newest deal on the board. ${hero?.name?`${hero.name} is the headline name, but the full package tells a deeper story about two front offices moving in different directions.`:'The move carries immediate implications for roster construction and the next wave of negotiations.'} Early trade grades suggest the deal is competitive, yet the real verdict will depend on whether the incoming centrepieces can deliver when the season pressure rises.`});}
  if(hotTeam){const hotName=managerName(hotTeam.id),oddsLabel=hotOdds&&!hotOdds.eliminated?championshipOddsLabel(hotOdds):'long odds',stake=[25000,50000,75000][Math.min(2,Math.max(0,hotTeam.count-3))];stories.push({kicker:'MARKET BUZZ',headline:`A punter has reportedly placed $${stake.toLocaleString()} on ${hotName} to win it all`,image:state.managers.get(hotTeam.id)?.avatar||null,body:`The betting window around IMO Dynasty has suddenly become a lot more interesting. After ${hotName} ripped off ${hotTeam.count} consecutive wins, league chatter claims one bold punter has backed the streaking franchise at ${oddsLabel}. It is exactly the kind of speculative wager that turns a strong run into a full-blown storyline. Rival GMs may dismiss it as heat-of-the-moment hype, but another win would force the market to take the surge seriously — and could make today's price look very different by the next edition.`});}
  if(leader&&stories.length<3){const move=(state.previousPowerRanks[leader.id]||leader.rank)-leader.rank;stories.push({kicker:'POWER SURGE',headline:`${leader.name} ${move>0?'storms':'holds'} the No. 1 spot`,image:state.managers.get(String(leader.id))?.avatar||null,body:`${leader.name} sits atop the latest IMO Dynasty Power Rankings, combining current results, recent form and championship outlook into the strongest profile in the league. ${move>0?`A rise of ${move} place${move===1?'':'s'} has turned steady progress into a genuine statement.`:'The challenge now is sustaining that position as rivals begin to adjust.'} Every week at the summit increases the pressure, but it also confirms that this roster has become the standard the rest of the league is chasing.`});}
  if(topPlayer&&stories.length<3){const owner=currentRosterOwner(topPlayer.id);stories.push({kicker:'AWARD WATCH',headline:`${topPlayer.name} sets the early fantasy pace`,image:insiderPlayerImage(topPlayer.id),body:`At ${topPlayer.avg.toFixed(1)} fantasy points per game, ${topPlayer.name} currently owns one of the strongest statistical cases in the league. ${owner?`${managerName(owner)} has built a major weekly advantage around that production, and opponents are already treating every matchup as a test of how to contain it.`:'The numbers have pushed him firmly into the awards conversation.'} The next gameweek will reveal whether this is simply elite form or the beginning of a season-defining run.`});}
  ...
  const edition={eventKey,issue,date:new Date().toISOString(),stories:stories.slice(0,3),coming:coming.slice(0,3)};try{localStorage.setItem(storageKey,JSON.stringify(edition))}catch{}return edition;
}
function renderHeadlines(){const root=$('headlinesContent');if(!root)return;const ed=buildInsiderEdition();root.innerHTML=`<header class="insider-header"><div><span class="eyebrow">📰 IMO INSIDER</span><h2 id="headlinesTitle">Today's Headlines</h2><p>${fmt(new Date(ed.date).getTime())} · Issue #${String(ed.issue).padStart(3,'0')}</p></div><span class="insider-live">LATEST EDITION</span></header><div class="insider-stories">${ed.stories.map((story,i)=>`<article class="insider-story">${story.image?`<div class="insider-image"><img src="${esc(story.image)}" alt="" loading="lazy" onerror="this.parentElement.remove()"></div>`:''}<div class="insider-copy"><span>${esc(story.kicker)}</span><h3>${esc(story.headline)}</h3><p>${esc(story.body)}</p><small>${i===0?'3 min read':'2 min read'}</small></div></article>`).join('')}</div><aside class="coming-tomorrow"><span>COMING TOMORROW</span><ul>${ed.coming.map(x=>`<li>${esc(x)}</li>`).join('')}</ul></aside>`}
