...
const state={league:null,currentUsers:[],currentRosters:[],managers:new Map(),trades:[],selectedWindow:"14",players:{},bundles:[],modelBundle:null,playerAverages:{},previousPowerRanks:{},heatmapExpanded:false,draftPickMap:{},previousPlayerAverages:{},votePlayers:[],activeWindow:"14",biggestTradesExpanded:false,profileAverageSeason:"2025"};
...
function activeWindowLabel(w){return w==="season"?"2026 season":WL[w]}
...
function recentPlayerForm(){
  ...
  return {
    poor:rows.filter(x=>x.priorAvg>=20&&x.recentAvg>=10&&x.recentAvg>0&&x.change<0).sort((a,b)=>a.change-b.change).slice(0,5),
    good:rows.filter(x=>x.priorAvg>=15&&x.recentAvg>=18&&x.recentAvg>0&&x.change>0).sort((a,b)=>b.change-a.change).slice(0,5)
  }
}
...
function seasonAverageMap(season){const bundle=bundleForSeason(season);if(!bundle)return{};const weeks=meaningfulWeeks(bundle);if(!weeks.length)return{};return buildPlayerAverages(bundle,weeks.at(-1))}
...
function managerBadges(managerId){
  const id=String(managerId),badges=[],trades=managerTrades(id),months={};
  trades.forEach(t=>{const d=new Date(t.created||0),key=`${d.getFullYear()}-${d.getMonth()}`;months[key]=(months[key]||0)+1});
  if(Object.values(months).some(n=>n>=5))badges.push({icon:"⚡",name:"The Day Trader",copy:"Completed five or more trades in a single month."});
  const firsts=upcomingFirstCount(id);if(firsts>=4)badges.push({icon:"🧰",name:"Draft Capital Hoarder",copy:`Currently controls ${firsts} upcoming first-round picks.`});
  const seasonTrades=currentSeasonTrades().filter(t=>mids(t).includes(id)),partners=new Set(seasonTrades.flatMap(t=>mids(t).filter(x=>x!==id)));if(state.managers.size>1&&partners.size>=state.managers.size-1)badges.push({icon:"🤝",name:"The Negotiator",copy:"Has traded with every other GM this season."});
  if(championManagerIds().has(id))badges.push({icon:"🏆",name:"Champion",copy:"Has won an IMO Dynasty Cup championship."});
  ...
  const avgMap=seasonAverageMap("2025"),topPlayer=Object.entries(avgMap).sort((a,b)=>b[1]-a[1])[0]?.[0],manager=state.managers.get(id);if(topPlayer&&(manager?.roster?.players||[]).map(String).includes(String(topPlayer)))badges.push({icon:"👑",name:"MVP",copy:`Currently owns ${playerName(topPlayer)}, the league's highest-average player.`});
  return badges
}
function managerProfileHTML(managerId){
  ...
  const badgeRows=badges.map(b=>`<div class="gm-badge"><span>${b.icon}</span><div><strong>${esc(b.name)}</strong><small>${esc(b.copy)}</small></div></div>`).join("")||'<div class="profile-empty">No badges earned yet.</div>';
  const managerAvatar=manager.avatar?`<img src="${esc(manager.avatar)}" alt="">`:esc(manager.initials||manager.name.slice(0,2).toUpperCase());
  return `<header class="manager-profile-hero"><div class="manager-profile-avatar">${managerAvatar}</div><div><span class="eyebrow">MANAGER PROFILE</span><h2 id="managerProfileTitle">${esc(manager.name)}</h2><p>Current franchise overview and league history</p></div></header>
  <div class="manager-profile-stat-grid"><div><span>Power rank</span><strong>${power?`#${power.rank}`:"—"}</strong></div><div><span>Ladder</span><strong>${power?`#${power.standingRank}`:"—"}</strong></div><div><span>Record</span><strong>${form.games?`${form.wins}-${form.games-form.wins}`:"—"}</strong></div><div><span>Championship odds</span><strong>${odds?`$${odds.odds.toFixed(2)}`:"—"}</strong></div><div><span>Last 5 average</span><strong>${form.avg5?form.avg5.toFixed(1):"—"}</strong></div><div><span>Career trades</span><strong>${trades.length}</strong></div></div>
  <section class="manager-profile-card achievement-wall"><div class="manager-profile-card-heading"><div><span class="eyebrow">GM IDENTITY</span><h3>Badges & Achievement Wall</h3></div><span class="period-pill">${badges.length} earned</span></div><div class="gm-badge-grid">${badgeRows}</div></section>
  <div class="manager-profile-grid">
    ...
    <section class="manager-profile-card"><div class="manager-profile-card-heading"><div><span class="eyebrow">CURRENT FORM</span><h3>Last Five</h3></div></div><div class="profile-form-strip">${formPills}</div><div class="profile-form-copy"><strong>${form.streak?`${form.streak}-game winning streak`:`${form.recent.reduce((s,g)=>s+g.result,0)} wins from the last ${form.recent.length}`}</strong><span>${form.avg5?`${form.avg5.toFixed(1)} average team score`:"Season has not started"}</span></div></section>
